...
def main(irc):
    ...
    while run:
        ...
        if 'Candy!Candy@root-me.org PRIVMSG Rither' in res:
            ...
            res = res.replace("r :", "")
            result = base64.b64decode(res).decode()
            print("REPLYING ANSWER:  %s :!ep2 -rep %s" % (bot, result))
            irc.send(get_msg("PRIVMSG %s :!ep2 -rep %s" % (bot, result)))
        cmd = input('command: ')
        if cmd == 'e':
            run = False
        elif cmd == 's':
            irc.send(get_msg("PRIVMSG %s :!ep2" % bot))
